...
export const CreateInvoice = () => {
  ...
  const [{ customer, image, vehName }, setInvoice] = useState<
    IInvoice<File | null>
  >({
    customer: "",
    image: null,
  ...
  const onChange = (
    key: keyof IInvoice<File>,
    value: IInvoice<File>[keyof IInvoice<File>],
  ) => {
  ...
  const handleSubmit = async (event: React.SyntheticEvent): Promise<void> => {
    ...
    const [success, result, data] = await new Invoice(
      customer,
      image,
      vehName,
    ).create(user.getIdentifier)
    if (!success) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-sm ">
        <DialogHeader>
          ...
          <DialogDescription>{t("invoices.enter_info")}</DialogDescription>
        </DialogHeader>
        <form
          id="dialog"
          className="gap-6"
